...
@property(strong, nonatomic) NSMutableArray *wantToMeetThem;
@property(strong, nonatomic) NSMutableArray *myFavorites;
@property(strong, nonatomic) NSMutableArray *maybeWantToMeet;
@property(strong, nonatomic) NSMutableDictionary *sections;
@property(strong, nonatomic) NSMutableArray *ar_selectedCell;

@end

@implementation VCMyLink
BOOL isEditing;
@synthesize /*wantToMeetMe,*/ wantToMeetThem, myFavorites, mutualMatches, sections, toolbar;
...
-(void)loadDataForAllList{
    ...
    [Profile getListPeople:URL_getListMyFavorites handler:^(NSMutableArray *list, int count) {
        self.myFavorites = list;
        [self.sections setObject:self.myFavorites forKey:@"2"];
        [self.gridView reloadData];
    }];
    
    [Profile getListPeople:URL_getListIWantToMeet handler:^(NSMutableArray *list, int count) {
        self.wantToMeetThem = list;
        //        [self.sections replaceObjectAtIndex:1 withObject:self.wantToMeetThem];
        [self.sections setObject:self.wantToMeetThem forKey:@"1"];
        [self.gridView reloadData];
    }];
    ...
    [Profile getListPeople:URL_getListMutualMatch handler:^(NSMutableArray *list, int count) {
        ...
        [self.sections setObject:self.mutualMatches forKey:@"0"];
        [self.gridView reloadData];
    }];
    
    [Profile getListPeople:URL_getListMaybeIWantToMeet handler:^(NSMutableArray *list, int count) {
        self.maybeWantToMeet = list;
        //        [self.sections replaceObjectAtIndex:0 withObject:self.mutualMatches];
        [self.sections setObject:self.maybeWantToMeet forKey:@"3"];
        [self.gridView reloadData];
    }];
    /*
    // =====> MyFavorited List
    NSDictionary *params_checkedmeout = [[NSDictionary alloc]initWithObjectsAndKeys:@"0",@"start",@"999",@"limit", nil];
    [request getPath:URL_getListMyFavorites parameters:params_checkedmeout success:^(__unused AFHTTPRequestOperation *operation, id JSON) {
        self.myFavorites = [Profile parseProfileToArrayByJSON:JSON];
        [self.sections setObject:self.myFavorites forKey:@"2"];
        [self.gridView reloadData];
    ...
    // =====> WhoIWantToMeetThem List
    NSDictionary *params_wanttomeet = [[NSDictionary alloc]initWithObjectsAndKeys:@"0",@"start",@"999",@"limit", nil];
    [request getPath:URL_getListIWantToMeet parameters:params_wanttomeet success:^(__unused AFHTTPRequestOperation *operation, id JSON) {
        self.wantToMeetThem = [Profile parseProfileToArrayByJSON:JSON];
//        [self.sections replaceObjectAtIndex:1 withObject:self.wantToMeetThem];
        [self.sections setObject:self.wantToMeetThem forKey:@"1"];
        [self.gridView reloadData];
    } failure:^(AFHTTPRequestOperation *operation, NSError *error) {
    ...
    // =====> MutualMatches List
    NSDictionary *params_mutulmatches = [[NSDictionary alloc]initWithObjectsAndKeys:@"0",@"start",@"999",@"limit", nil];
    [request getPath:URL_getListMutualMatch parameters:params_mutulmatches success:^(__unused AFHTTPRequestOperation *operation, id JSON) {
        self.mutualMatches = [Profile parseProfileToArrayByJSON:JSON];
//        [self.sections replaceObjectAtIndex:0 withObject:self.mutualMatches];
        [self.sections setObject:self.mutualMatches forKey:@"0"];
        [self.gridView reloadData];
    } failure:^(AFHTTPRequestOperation *operation, NSError *error) {
    ...
    // =====> MaybeIWantToMeet List
    NSDictionary *params_maybe = [[NSDictionary alloc]initWithObjectsAndKeys:@"0",@"start",@"999",@"limit", nil];
    [request getPath:URL_getListMaybeIWantToMeet parameters:params_maybe success:^(__unused AFHTTPRequestOperation *operation, id JSON) {
        self.maybeWantToMeet = [Profile parseProfileToArrayByJSON:JSON];
        //        [self.sections replaceObjectAtIndex:0 withObject:self.mutualMatches];
        [self.sections setObject:self.maybeWantToMeet forKey:@"3"];
        [self.gridView reloadData];
    } failure:^(AFHTTPRequestOperation *operation, NSError *error) {
...
- (UIView *) gridView:(UIGridViewMultipleSection *)grid viewForHeaderInSection:(NSInteger)section {
    ...
    switch (section) {
        case 0:
            ...
            break;
        case 1:
            headerName = [NSString stringWithFormat:@"\tI want to meet (%d)", [self.wantToMeetThem count]];
            break;
        case 2:
            headerName = [NSString stringWithFormat:@"\tMy favorites (%d)", [self.myFavorites count]];
            break;
        case 3:
            headerName = [NSString stringWithFormat:@"\tMaybe I want to meet (%d)", [self.maybeWantToMeet count]];
            break;
        default:
